function SI_one_slice = simu_noTB_with_phase_interleaving_2_SMS_3(flip_angle_slice,slice_phase_excitation,Mz,M0,TR,Slab_T1,Nalpha)
%--------------------------------------------------------------------------
%   SI_one_slice = simu_noTB_with_phase_interleaving_2_SMS_3(flip_angle_slice,slice_phase_excitation,Mz,M0,TR,Slab_T1,Nalpha)
%--------------------------------------------------------------------------
...
SMS_slices = [1:1000,401:1400,801:1800]; slice_gap = 200;
...
for i=1:Nalpha
    ...
    flip_angle_all(slice_position(1:1000)) = flip_angle_all(slice_position(1:1000)) + flip_angle_slice;
    flip_angle_all(slice_position(1001:2000)) = flip_angle_all(slice_position(1001:2000)) + flip_angle_slice;
    flip_angle_all(slice_position(2001:3000)) = flip_angle_all(slice_position(2001:3000)) + flip_angle_slice;
    ...
    for iii = [1,3,5]-1+set(i)
        ...
        slice_location = (1:1000) + slice_gap*(iii-1);

        SI_set(set(i),N) = SI_set(set(i),N) + sum(Mz(slice_location,end-1).*sin(flip_angle_slice/180*pi).*slice_phase_excitation) .*exp(1i*n_phase(n_SMS_slice)*2*pi/3);
    end
