function [SI_one_slice,Mz] = simu_TB_with_phase_interleaving_2_SMS_3_with_motion(flip_angle_slice_excitation,flip_angle_slice_transition,slice_phase_excitation,Mz,M0,TR,Slab_T1,Nalpha,max_disp,heart_rate)
%--------------------------------------------------------------------------
...
%   with transition bands acquisition to generate a signal dictionary for
%   motion simulations
%--------------------------------------------------------------------------
%   Inputs:      
...
SMS_slices = [1:1000,401:1400,801:1800] + 1400; slice_gap = 200;
transition_position = [1051:2050;2751:3750];
...
for i=1:Nalpha
    ...
    if set(i) == 2
        ...
        flip_angle_all(transition_position(1,:) + Motion(i)) = flip_angle_all(transition_position(1,:) + Motion(i)) + flip_angle_slice_transition;
    else
        flip_angle_all(transition_position(2,:) + Motion(i)) = flip_angle_all(transition_position(2,:) + Motion(i)) + flip_angle_slice_transition;
    end
    ...
    for iii = [1,3,5]-1+set(i)
        ...
        slice_location = (1:1000) + 1400 + slice_gap*(iii-1) + Motion(i);
