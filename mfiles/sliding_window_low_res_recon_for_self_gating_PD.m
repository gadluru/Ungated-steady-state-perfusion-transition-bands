function [Image, para] = sliding_window_low_res_recon_for_self_gating_PD(kSpace_all,para)
%--------------------------------------------------------------------------
%   [Image, para] = sliding_window_low_res_recon_for_self_gating_PD(kSpace_all,para)
%--------------------------------------------------------------------------
%   Function to perform STCR reconstruction with LLR constraint for Proton
%   Density Data
%--------------------------------------------------------------------------
%   Inputs:      
...
%--------------------------------------------------------------------------
%   Outputs:
%       - Image: Reconstructed Proton Density Images [nx,ny,nt,nset,nsl]
...
kSpace_all = kSpace_all(:,1:para.kSpace_info.NumberOfPDlines,:);
para.kSpace_info.set = para.kSpace_info.set(1:para.kSpace_info.NumberOfPDlines);
para.kSpace_info.angle_mod = para.kSpace_info.angle_mod(1:para.kSpace_info.NumberOfPDlines);
para.kSpace_info.phase_mod = para.kSpace_info.phase_mod(1:para.kSpace_info.NumberOfPDlines);
para.kSpace_info.TimeStamp = para.kSpace_info.TimeStamp(1:para.kSpace_info.NumberOfPDlines);
...
for i=1:para.Recon.nset
    ...
    for j=1:nof
        ray_idx = (j-1)*para.Recon.nor_sl+1:(j-1)*para.Recon.nor_sl+para.Recon.nor;
        kSpace(:,:,j,:) = kSpace_set(:,ray_idx,:);
        theta(1,:,j) = theta_set(ray_idx);
        phase(1,:,j) = phase_set(ray_idx);
    end
...
siz = size(Data{1}.first_est); siz(4) = para.Recon.nset; Image = zeros(siz,'single'); para.Recon.tau = 5; para.Recon.noi = 100;
for i=1:para.Recon.nset
    scale = max(abs(Data{i}.first_est(:)));
    para.Recon.weight_tTV = scale*para.weight_tTV;
    para.Recon.weight_sTV = scale*para.weight_sTV;
    Image(:,:,:,i,:) = STCR_conjugate_gradient_nSMS_low_rank(Data{i},para);
end
