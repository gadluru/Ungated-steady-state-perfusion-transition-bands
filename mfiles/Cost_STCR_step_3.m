function [Cost,totalCost] = Cost_STCR_step_3(Image, l2ref, fNorm, llr, para)
%--------------------------------------------------------------------------
%   [Cost,totalCost] = Cost_STCR_step_3(Image, l2ref, fNorm, llr, para)
%--------------------------------------------------------------------------
%   computes the cost of the ADMM Step 3 reconstruction problem 
%--------------------------------------------------------------------------
%   Inputs:
...
%       - l2ref: l2 norm used for conjugate gardient step sizes [nx,ny,nt,nset*nsl]
%               - nx: number of measurements in spatial x-dimension
%               - ny: number of measurements in spatial y-dimension
%               - nt: number of time frames after cardiac phase fixing
%               - nset: number of slice groups
...
%       - totalCost: total cost of the current step of the ADMM Step 3
%               reconstruction
%--------------------------------------------------------------------------
...
if para.Recon.weight_l2
    l2Norm = para.Recon.weight_l2 * sum(abs(vec(Image - l2ref)).^2);
else
    l2Norm = 0;
end
...
totalCost = slNorm + sNorm + tNorm + fNorm + l2Norm;
...
if isempty(Cost.fidelityNorm)
    ...
    Cost.l2Norm(end+1) = gather(l2Norm);
